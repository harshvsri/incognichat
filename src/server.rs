...
use std::fmt::{self, Display, Write as OtherWrite};
...
// `Self` refers to the type of the struct, while `self` is the actual instance (object) of the struct being used.
impl<T: Display> Display for Sens<T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let Self(inner) = self;
        if SAFE_MODE {
            // "[REDACTED]" replaces sensitive data to prevent exposure when SAFE_MODE is enabled.
            // Sensitive information has been intentionally hidden for security or privacy reasons.
            "[REDACTED]".fmt(f)
        } else {
            inner.fmt(f)
        }
...
fn generate_token() -> Result<String> {
    ...
    // Iterate over the buffer and write each element as a two-digit uppercase hexadecimal into the token.
...
fn main() -> Result<()> {
    // Token generation and saving it.
    ...
    let mut listener =
        TcpListener::bind(address.parse::<SocketAddr>().unwrap()).map_err(|err| {
            eprintln!(
